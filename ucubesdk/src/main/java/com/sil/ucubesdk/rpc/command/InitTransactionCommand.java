...
public class InitTransactionCommand extends RPCCommand {
	...
	public InitTransactionCommand(String amount, @NonNull Currency currency, byte transactionType, @NonNull EMVApplicationDescriptor application) {
	...
	protected byte[] createPayload() {
		...
		payload[offset++] = 0x02;
		payload[offset++] = 0x06;
		System.arraycopy(Tools.toBCD_double(Double.parseDouble(amount) * Math.pow(10, currency.getExponent()), 6), 0, payload, offset, 6);
		offset += 6;
		...
		if (transactionType == Constants.PURCHASE_CASHBACK) {
			...
			payload[offset++] = 0x03;
			payload[offset++] = 0x06;
			Log.d("CASHBKAMT1",""+cashbackAmount);
